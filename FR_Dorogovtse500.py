...
def fruchterman_reingold_layout(grafo, width, height, positions, iterations=50, k=None):
    if k is None:
        k = math.sqrt((width/3 * height/3) / len(grafo.nodos))
    t = width / 10
    dt = t / iterations
    area = width * height
...
def draw_graph_fruchterman_reingold(grafo, positions, width=800, height=600, delay=10, iterations_per_frame=10):
    ...
    with imageio.get_writer('FR_DorogovtseMendes500.mp4', fps=10) as writer:
...
if __name__ == "__main__":
    ...
    draw_graph_fruchterman_reingold(grafo, positions, width, height, delay=10, iterations_per_frame=10)
